...
void dehazing::AirlightEstimation(IplImage* imInput)
{
	...
	if(nHei*nWid > 200)
	{
		...
		Mat temp;
		temp= cvarrToMat(iplUpperLeft);
		vector<Mat> splt;
		split(temp, splt);
		iplR = &IplImage(splt[2]);
		iplG = &IplImage(splt[1]);
		iplB = &IplImage(splt[0]);

		Mat dpMean, dpStds;
		meanStdDev(splt[2], dpMean, dpStds);
		dpScore[0] = dpMean.at<double>(0, 0) - dpStds.at<double>(0, 0);
		meanStdDev(splt[1], dpMean, dpStds);
		dpScore[1] = dpMean.at<double>(0, 0) - dpStds.at<double>(0, 0);
		meanStdDev(splt[0], dpMean, dpStds);
		dpScore[2] = dpMean.at<double>(0, 0) - dpStds.at<double>(0, 0);
...
/*
	...
	Return:
		imOutput - output image
 */
void dehazing::ImageHazeRemoval(Mat Input, Mat &Output){

	IplImage* imInput = &IplImage(Input);
	Output = Input.clone();
	IplImage* imOutput = &IplImage(Output);
	IplImage* imAir;
...
void dehazing::IplImageToInt(IplImage* imInput)
{
	...
	for (nY = 0; nY < m_nHei; nY++)
	{
		for (nX = 0; nX < m_nWid; nX++)
		{
			// (1) IplImage 甫 YUV狼 Y盲澄肺 函券 窍咯 int屈 硅凯 m_pnYImg俊 历厘
...
void dehazing::DownsampleImage()
{
	...
	for (nY = 0; nY < 240; nY++)
	{
		for (nX = 0; nX < 320; nX++)
		{
			// (1) 糕滚 函荐牢 m_pnYImg甫 m_pnSmallYImg肺 促款基敲傅(农扁绰 320x240)
...
void dehazing::UpsampleTransmission()
{
	...
	for (nY = 0; nY < m_nHei; nY++)
	{
		for (nX = 0; nX < m_nWid; nX++)
		{
			// (1) 糕滚 函荐牢 m_pfSmallTransR甫 m_pfTransmission肺 诀基敲傅
